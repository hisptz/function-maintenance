...
export class FunctionComponent implements OnInit {
  ...
  loadFunction() {
    return new Observable(observable => {
      if (this.id === 'new') {
        this.functionService.getId().subscribe(
          (results: any) => {
        ...
        this.functionService.get(this.id).subscribe(
          (func: any) => {
            this.func = func;
            observable.next();
            observable.complete();
  ...
  save(goBack) {
    ...
    if (this.func.name && this.func.name !== '') {
      ...
      this.functionService.save(this.func).subscribe(
        results => {
          this.func = results;
  ...
  ruleDetails = [];
  editRule(rule) {
    const selectedRule = Object.assign({}, rule);
  ...
  removeRule(rule) {
    const index = this.ruleDetails.indexOf(rule);
