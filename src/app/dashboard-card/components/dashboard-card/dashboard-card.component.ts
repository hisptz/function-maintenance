...
export const VISUALIZATION_WITH_NO_OPTIONS = [
  'USERS',
  'REPORTS',
  'RESOURCES',
  'APP'
...
export class DashboardCardComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.visualizationObject = {
      ...
      name: this.cardData.hasOwnProperty('displayName')
        ? this.cardData.displayName
        : null,
      type: this.cardData.hasOwnProperty('type') ? this.cardData.type : null,
      created: this.cardData.hasOwnProperty('created')
        ? this.cardData.created
        : null,
      lastUpdated: this.cardData.hasOwnProperty('lastUpdated')
        ? this.cardData.lastUpdated
        : null,
      shape: this.cardData.hasOwnProperty('shape')
        ? this.cardData.shape
        : 'NORMAL',
      details: {
        cardHeight: this.cardConfiguration.defaultHeight,
        itemHeight: this.cardConfiguration.defaultItemHeight,
        currentVisualization: this.cardData.hasOwnProperty('type')
          ? this.cardData.type
          : null,
        externalDimensions: this.externalDimensions,
        externalLayout: this.externalLayout,
        cardStatusId: Math.random()
          .toString(36)
          .substr(2, 5)
      },
      layers: [],
    ...
    if (this.externalAnalytics != null) {
      this.visualizationObject.layers.push({
        settings: {},
        analytics: this.externalAnalytics
      });
    }
    ...
    if (this.visualizationObject.type != null) {
      if (
        this.cardData.hasOwnProperty(
          this.utilities.camelCaseName(this.visualizationObject.type)
        )
      ) {
        let favoriteData = this.cardData[
          this.utilities.camelCaseName(this.visualizationObject.type)
        ];
  ...
  hideOptions(
    visualizationType,
    visualizationWithNoOptions: any[] = []
  ): boolean {
    let hide = false;
    if (
      visualizationWithNoOptions != undefined &&
      visualizationWithNoOptions.length > 0
    ) {
  ...
  resizeDashboard(currentShape: string, shapes: any[] = []): void {
    ...
    if (shapes != undefined && shapes.length > 1) {
      shapes.forEach((shapeValue, shapeIndex) => {
        if (
          shapeValue.hasOwnProperty('shape') &&
          shapeValue.shape == currentShape
        ) {
