...
export class AnalyticsService {
  ...
  public getAnalytics(
    ...
    let url = externalSource
      ? this._constructAnalyticUrlForExternalSource(favoriteObject)
      : this._constructAnalyticUrl(favoriteObject, favoriteType, customFilter);
  ...
  private _constructAnalyticUrl(
     ...
     * @type {string}
     */
    let rowParameters = this._getDimension(
      'rows',
      favoriteObject,
      customFilter
    );
    let columnParameters = this._getDimension(
      'columns',
      favoriteObject,
      customFilter
    );
    let filterParameters = this._getDimension(
      'filters',
      favoriteObject,
      customFilter
    );
    ...
    if (favoriteType == 'EVENT_CHART') {
      ...
      if (favoriteObject.hasOwnProperty('dataType')) {
        if (favoriteObject.dataType == 'AGGREGATED_VALUES') {
          url +=
            '/events/aggregate/' + this._getProgramParameters(favoriteObject);
        } else {
       ...
       */
      if (
        favoriteObject.hasOwnProperty('startDate') &&
        favoriteObject.hasOwnProperty('endDate')
      ) {
        url +=
          'startDate=' +
          favoriteObject.startDate +
          '&' +
          'endDate=' +
          favoriteObject.endDate +
          '&';
      }
    } else {
      url += '.json?';
     ...
     */
    url +=
      '&displayProperty=NAME' + favoriteType == 'EVENT_CHART'
        ? '&outputType=EVENT&'
  ...
  private _getProgramParameters(favoriteObject: any): string {
    let params: string = '';
    if (
      favoriteObject.hasOwnProperty('program') &&
      favoriteObject.hasOwnProperty('programStage')
    ) {
      if (
        favoriteObject.program.hasOwnProperty('id') &&
        favoriteObject.programStage.hasOwnProperty('id')
      ) {
        params =
          favoriteObject.program.id +
          '.json?stage=' +
          favoriteObject.programStage.id +
          '&';
      } else {
  ...
  private _getDimension(
    ...
    if (favoriteObject.hasOwnProperty(dimension)) {
      favoriteObject[dimension].forEach((dimensionValue: any) => {
        items += items != '' ? '&' : '';
        if (
          dimensionValue.hasOwnProperty('dimension') &&
          dimensionValue.dimension != 'dy'
        ) {
          ...
          if (
            dimensionValue.hasOwnProperty('items') &&
            dimensionValue.items.length > 0
          ) {
            /**
             * Get custom value from filter if supplied
             * @type {string}
             */
            let customDimensionValue = this._getCustomDimensionValue(
              customFilter,
            ...
            if (customDimensionValue != null) {
              ...
              dimensionValue.items.forEach((itemValue, itemIndex) => {
                ...
                items +=
                  itemIndex == dimensionValue.items.length - 1 ? '' : ';';
              });
            }
          }
        } else {
