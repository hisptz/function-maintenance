...
export class Store {
  ...
  public select(
    ...
    return Observable.create(observer => {
      if (this.dataStore[item].length > 0) {
        ...
        this.loadAndSave(item, url, 'array', null, resultKey).subscribe(
          data => {
            observer.next(data);
            observer.complete();
          },
  ...
  public selectById(
    ...
    return Observable.create(observer => {
      ...
      if (result.length == 0) {
        this.loadAndSave(item, url, resultType, id, resultKey).subscribe(
          data => {
            let result =
              resultType == 'array'
                ? data.filter(dataValue => dataValue.id == id)[0]
                : data;
