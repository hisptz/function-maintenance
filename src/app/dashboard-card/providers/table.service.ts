...
export class TableService {
      ...
      if (tableDataFromStore != null) {
        tableDataFromStore.layers.forEach(layer => {
          if (!layer.settings.hasOwnProperty('tableConfiguration')) {
            layer.settings.tableConfiguration = this._getTableConfiguration(
              layer.settings,
              tableDataFromStore.type
            );
          }
        });
        ...
        if (tableData.details.hasOwnProperty('favorite')) {
          let favoriteType = tableData.details.favorite.hasOwnProperty('type')
            ? tableData.details.favorite.type
            : null;
          let favoriteId = tableData.details.favorite.hasOwnProperty('id')
            ? tableData.details.favorite.id
            : null;
            ...
            this.http
              ...
              .subscribe(
                (favoriteResponse: any) => {
                   ...
                   */
                  favoriteResponse.tableConfiguration = this._getTableConfiguration(
                    favoriteResponse,
                    tableData.type
                  );
          ...
          tableData.details.hasOwnProperty('externalDimensions') &&
          tableData.layers.length == 0
        ) {
          let nullItems: any[] = [];
          ...
          if (nullItems.length > 0) {
            ...
            this.analyticsService
              .getAnalytics(
                tableData.details.externalDimensions,
                null,
                [],
                true
              )
              .subscribe(
                analyticResponse => {
                   ...
                   */
                  let settings: any = {};
                  settings.tableConfiguration = this._getTableConfiguration(
                    {},
                    tableData.type,
                    tableData.details.externalLayout
                  );
                  /**
                   * Update table data with new information
                   */
  ...
  private _getTableConfiguration(
    favoriteObject,
    favoriteType,
    externalLayout: any = null
  ): TableConfiguration {
    /**
     * Get columns
     * @type {Array}
    ...
    if (favoriteObject.hasOwnProperty('columns')) {
      ...
      externalLayout != null &&
      externalLayout.hasOwnProperty('columns')
    ) {
    ...
    if (favoriteObject.hasOwnProperty('rows')) {
      ...
      externalLayout != null &&
      externalLayout.hasOwnProperty('rows')
    ) {
    ...
    if (tableData.layers.length > 0) {
      tableData.layers.forEach(layer => {
        if (layer.analytics.hasOwnProperty('headers')) {
          tableObjects.push(
            this.visualizationService.drawTable(
              layer.analytics,
              layer.settings.tableConfiguration
            )
          );
        } else {
