...
export class DashboardItemComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    if (
      this.currentVisualization == 'CHART' ||
      ...
      this.dashboardService
        .getMapObject(this.itemData.id, this.dashboardId)
        .subscribe(mapObject => {
  ...
  visualize(dashboardItemType, dataArray) {
    ...
    if (dashboardItemType == 'CHART' || dashboardItemType == 'EVENT_CHART') {
      ...
      dashboardItemType == 'EVENT_REPORT' ||
      dashboardItemType == 'REPORT_TABLE'
    ) {
      ...
      if (dataArray.basemap) {
        ...
        let orgunitString = this.dashboardService.getOrganisationUnitString(
          this.itemData.object,
          'ou'
        );
        this.dashboardService
          .getGeoFeature(orgunitString)
          .subscribe(feature => {
            this.displayMap = 'visible';
            this.loadingMap = false;
            this.dashboardService
              .convertToMapObject(this.itemData)
              .subscribe(response => {
                dataArray = response;
                this.drawMap(dataArray);
              });
  ...
  setVisualization(visualizationType) {
    ...
    this.getVisualizationArray(this.itemData, visualizationType).subscribe(
      dataArray => {
    ...
    return Observable.create(observer => {
      ...
      if (visualizationType != 'MAP') {
        if (itemData.hasOwnProperty('mapObject')) {
          ...
          dataArray = [
            { object: itemData.object, analytic: itemData.analytic }
          ];
        }
      } else {
        if (itemData.hasOwnProperty('mapObject')) {
  ...
  drawChart(dataArray: any[]) {
    ...
    dataArray.forEach(data => {
      ...
      if (!this.utility.isNull(this.customLayout)) {
        ...
        layout['series'] = data.object.series
          ? data.object.series
          : data.object.columns[0].dimension;
        layout['category'] = data.object.category
          ? data.object.category
          : data.object.rows[0].dimension;
      }
      ...
      chartObjects.push(
        this.visualizationService.drawChart(data.analytic, chartConfiguration)
      );
    });
  ...
  drawTable(dataArray) {
    ...
    dataArray.forEach(data => {
      ...
      tableObjects.push(
        this.visualizationService.drawTable(data.analytic, config)
      );
    });
  ...
  drawMap(mapObject) {
    this.baseMaps = {
      ...
      'OpenStreetMap Black&White': L.tileLayer(
  ...
  resizeDashboard(currentShape) {
    ...
    this.dashboardService.updateShape(
      this.dashboardId,
      this.itemData.id,
      newShape
    );
  ...
  deleteDashboardItem(id) {
    this.dashboardService.deleteDashboardItem(this.dashboardId, id).subscribe(
      response => {
  ...
  updateDashboardCard(dimension) {
    ...
    if (this.currentVisualization != 'MAP') {
      this.updateDasboardItemForAnalyticTypeItems(
        this.itemData.id,
        dimension.length == 2 ? dimension : [dimension]
      );
    }
  }

  updateDasboardItemForAnalyticTypeItems(
    dashboardItemId,
    customDimensions = []
  ) {
    this.loadingChart = true;
      ...
      .subscribe(
        dashboardItem => {
          ...
          this.visualize(this.currentVisualization, [
            { object: dashboardItem.object, analytic: dashboardItem.analytic }
          ]);
  ...
  updateDashboardItemLayout(layout) {
    ...
    this.visualize(this.currentVisualization, [
      { object: this.itemData.object, analytic: this.itemData.object }
    ]);
  }
  ...
  autoplayInterpretation(dashboardItem) {
    ...
    if (interpretationLength > 0) {
      interval(4000).subscribe(value => {
        if (interpretationIndex <= interpretationLength - 1) {
          this.interpretation =
            dashboardItem.object.interpretations[interpretationIndex].text;
  ...
  getDataElements(analytics, thematicLayer, rows, organisationUnits) {
    ...
    analytics.metaData.dx.forEach(dataElementsUids => {
      dataElementCarrier = {
        name: metaDataNames[dataElementsUids],
        uid: dataElementsUids,
        organisationUnitScores: []
      };
    });

    if (organisationUnits) {
      organisationUnits.forEach(organisationUnit => {
        dataElementCarrier.organisationUnitScores.push(
          this.getDataValue(
            rows,
            dataElementCarrier.uid,
            organisationUnit,
            period
          )
        );
  ...
  getDataValue(rows, dataElement, organisationUnit, period) {
    ...
    let orgunitId = organisationUnit.id
      ? organisationUnit.id
      : organisationUnit;
    let orgunitName = organisationUnit.na
      ? organisationUnit.na
      : organisationUnit;
    if (rows.length > 0) {
      rows.forEach(row => {
  ...
  getDataLayer(mapObject) {
    ...
    if (mapObject.type == 'event') {
      ...
      layers.forEach((layer, layerIndex) => {
        ...
        let organisationUnitScores =
          layer[layerNameArray[0]].organisationUnitScore;
          ...
          featureCollection.features.forEach((geoJsonFeature, featureIndex) => {
            organisationUnitScores.forEach((score, scoreIndex) => {
              ...
              if (score.orgunit.id == geoJsonFeature.properties.id) {
                ...
                scoreIndex >= scoresCount - 1 &&
                inValue.indexOf(geoJsonFeature.properties.id) == -1
              ) {
                value = '';
              }
            ...
            geoJsonFeature.properties.legend = this.prepareDataLegend(
              layerObjects
            );
          });
          ...
          layer.on({
            click: event => {
              ...
              let popUpContent: string =
                ...
                '<table>' +
                "<tr><td style='color:#333!important;'>" +
                featureName +
                '</td></tr>' +
                "<tr><td style='color:#333!important;'>" +
                dataElementName +
            ...
            mouseover: event => {
              ...
              let toolTipContent: string =
                "<div style='color:#333!important;'>" +
                '<table>' +
                "<tr><th style='color:#333!important;'>" +
                featureName +
                '</th>';
              if (dataElementValue == '') {
  ...
  getClusterLayerName(dataLayer) {
    ...
    if (dataLayer) {
      if (
        dataLayer.dataElementDimensions &&
        dataLayer.dataElementDimensions.length > 0
      ) {
  ...
  prepareTopLayers(dataLayers: any) {
    ...
    dataLayers.forEach((dataLayer, dataLayerIndex) => {
      ...
      if (featureCollection.features.length > 0) {
        featureCollection.features.forEach((geoJsonFeature, featureIndex) => {
          if (
            dataLayer.uid == organisationUnitScores[featureIndex].dataElementId
          ) {
            geoJsonFeature.properties.dataelement = {};
            dataLayer.organisationUnitScores[
              featureIndex
            ].organisationUnitName = geoJsonFeature.properties.name;
            geoJsonFeature.properties.dataelement = {
            ...
            layer.on({
              click: event => {
                let clickedFeature: Feature<GeometryObject> =
                  event.layer.feature;
                let properties: any = clickedFeature.properties;
              ...
              mouseover: event => {
                let hoveredFeature: Feature<GeometryObject> =
                  event.layer.feature;
                let properties: any = hoveredFeature.properties;
                let featureName = properties.name;
                let dataElementName = properties.dataelement.name;
                ...
                layer.setStyle(
                  (feature: GeoJSON.Feature<GeoJSON.GeometryObject>) => {
                    ...
                    let color: any = () => {
                      ...
                      return (feature.properties as any).legend(
                        dataElementScore
                      );
                    };
                    let featureStyle: any = {
  ...
  prepareHTMLLegend(coloringArray, legend, dataLayer) {
    let div =
      '<div style="background-color:white;width:90px;height:140px;font-size: 9px;padding: 5px;z-index:-100!important;" class="legend-content">';
  ...
  prepareDataLegend(dataLayer: any) {
    ...
    var sortedDataContainer: Array<number> = dataContainer.sort(
      (n1, n2) => n1 - n2
    );
    ...
    let dataRange = {
      ...
      max: sortedDataContainer[sortedDataContainer.length - 1]
        ? sortedDataContainer[sortedDataContainer.length - 1]
        : sortedDataContainer[0],
    ...
    return (dataElementScore: any, counter: any) => {
      ...
      if (legendType == 'equalInterval') {
        legend = this.equalIntervalLegend(
          sortedDataContainer,
          dataRange,
          legendSetting
        );
      ...
      let leg = {};
      leg[dataLayer.id] = this.prepareHTMLLegend(
        coloringArray,
        legend,
        dataLayer
      );
          ...
          if (!countFound) {
            if (legendType == 'equalInterval') {
              if (value == legendElement.min && value != legendElement.max) {
                ...
                value == legendElement.max &&
                value == legendElement.min
              ) {
  ...
  produceColorClassRange(colorRange: any) {
    ...
    if (colorRange <= 9 && colorRange >= 0) {
      ...
      for (
        ...
        singleColorIndex < 100;
        singleColorIndex += counts
      ) {
  ...
  getStyle(feature: GeoJSON.Feature<GeoJSON.GeometryObject>) {
    let color: any = () => {
      let dataElementValue: number = (feature.properties as any).dataelement
        .value;
