...
export class Dhis2MenuComponent implements OnInit {
  ...
  ngOnInit() {
    this.getSystemSettings().subscribe((data: any) => {
      this.start_module = data.startModule;
      this.application_title = data.applicationTitle;
      this.application_style =
        'currentStyle' in data ? data.currentStyle : data.keyStyle;
      ...
      menu_css.setAttribute(
        'href',
        this.dhis2_url + '/dhis-web-commons/css/menu.css'
      );
      ...
      this.getUserSettings().subscribe(
        (userData: any) => {
          this.application_style =
            'keyStyle' in userData ? userData.keyStyle : this.application_style;
          ...
          element.setAttribute(
            'href',
            this.dhis2_url + '/dhis-web-commons/css/' + this.application_style
          );
  ...
  getSystemSettings() {
    ...
    return this.http.get(this.dhis2_url + '/api/systemSettings.json').pipe(
  ...
  getUserSettings() {
    //noinspection TypeScriptUnresolvedFunction
    return this.http.get(this.dhis2_url + '/api/userSettings.json').pipe(
      map((response: Response) => response.json()),
      catchError(this.handleError)
    );
  }
