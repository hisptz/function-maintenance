...
import {
  IActionMapping,
  TREE_ACTIONS
} from 'angular-tree-component/dist/models/tree-options.model';
...
export class OrgUnitFilterComponent implements OnInit {
  ...
  constructor(private http: Http, private orgunitService: OrgUnitService) {
  ...
  updateModelOnSelect(data) {
    if (!this.orgunit_model.show_update_button) {
      setTimeout(data => {
        console.log(
          'kefsafsa',
          this.getOrgUnitsForAnalytics(this.orgunit_model, false)
        );
    ...
    this.orgunitService.getOrgunitLevelsInformation().subscribe((data: any) => {
      ...
      this.orgunitService
        ...
        .subscribe(userOrgunit => {
          let level = this.orgunitService.getUserHighestOrgUnitlevel(
            userOrgunit
          );
          ...
          let orgunits = this.orgunitService.getuserOrganisationUnitsWithHighestlevel(
            level,
            userOrgunit
          );
          ...
          this.orgunitService.getInitialOrgunitsForTree(orgunits).subscribe(
            initial_data => {
              ...
              let fields = this.orgunitService.generateUrlBasedOnLevels(
                use_level
              );
              this.orgunitService
                ...
                .subscribe(
                  items => {
                    ...
                    for (let active_orgunit of this.orgunit_model
                      .selected_orgunits) {
                      this.activateNode(active_orgunit.id, this.orgtree);
                    }
                    this.prepareOrganisationUnitTree(
  ...
  deActivateNode(nodeId: any, nodes, event) {
    setTimeout(() => {
      let node = nodes.treeModel.getNodeById(nodeId);
      if (node) node.setIsActive(false, true);
    }, 0);
    if (event != null) {
  ...
  getProperPreOrgunitName(): string {
    ...
    if (this.orgunit_model.selection_mode == 'Group') {
      ...
      if (this.orgunit_model.selected_user_orgunit == 'USER_ORGUNIT_CHILDREN') {
        name =
          this.getOrgUnitName(this.orgunit_model.user_orgunits[0].id) +
          ' sub-units';
      }
      if (
        this.orgunit_model.selected_user_orgunit == 'USER_ORGUNIT_GRANDCHILDREN'
      ) {
        name =
          this.getOrgUnitName(this.orgunit_model.user_orgunits[0].id) +
          ' sub-x2-units';
  ...
  getOrgUnitsForAnalytics(orgunit_model: any, with_children: boolean): string {
    ...
    if (orgunit_model.selection_mode == 'Usr_orgUnit') {
      if (orgunit_model.user_orgunits.length == 1) {
        let user_orgunit = this.orgtree.treeModel.getNodeById(
          orgunit_model.user_orgunits[0].id
        );
        ...
        organisation_unit_analytics_string +=
          orgunit_model.selected_user_orgunit;
      }
    } else {
      // if there is only one organisation unit selected
      if (orgunit_model.selected_orgunits.length == 1) {
        let detailed_orgunit = this.orgtree.treeModel.getNodeById(
      ...
      if (orgunit_model.selection_mode == 'Level') {
        organisation_unit_analytics_string +=
          orgunit_model.selected_level + ';';
      }
      if (orgunit_model.selection_mode == 'Group') {
        organisation_unit_analytics_string +=
          orgunit_model.selected_group + ';';
      }
    }
  ...
  getOrgunitLevelsInformation() {
    return this.http
      .get(
        this.root_url +
          'api/organisationUnitLevels.json?fields=id,name,level&order=level:asc'
      )
      .pipe(
  ...
  getUserInformation() {
    return this.http
      .get(
        this.root_url +
          'api/me.json?fields=dataViewOrganisationUnits[id,level],organisationUnits[id,level]'
      )
      .pipe(
        map((response: Response) => response.json()),
        catchError(this.handleError)
  ...
  getInitialOrgunitsForTree(orgunits) {
    return this.http
      .get(
        this.root_url +
          'api/me.json?fields=organisationUnits[id,name,children[id,name]]&filter=id:in:[' +
          orgunits.join(',') +
          ']&paging=false'
      )
      .pipe(
