...
export class OrgUnitService {
  ...
  getUserInformation(priority = null) {
    if (priority === false) {
      ...
      return this.http
        .get('../../../api/me.json?fields=organisationUnits[id,name,level]')
        .pipe(
          map((response: Response) => response.json()),
          catchError(this.handleError)
        );
  ...
  getAllOrgunitsForTree(fields) {
    return this.http
      .get(
        '../../../api/organisationUnits.json?filter=level:eq:1&paging=false&fields=' +
          fields
      )
      .pipe(
        map((response: Response) => response.json()),
  ...
  getInitialOrgunitsForTree(orgunits) {
    return Observable.create(observer => {
      if (this.initial_orgunits != null) {
        ...
        this.http
          .get(
            '../../../api/organisationUnits.json?fields=id,name,children[id,name]&filter=id:in:[' +
              orgunits.join(',') +
              ']&paging=false'
          )
          .pipe(
