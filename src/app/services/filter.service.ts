...
export class FilterService {
  ...
  getOrgunitLevelsInformation() {
    return this.http
      .get(this.constant.root_url + 'api/organisationUnitLevels.json?fields=id')
      .pipe(
  ...
  getInitialOrgunitsForTree(uid: string = null) {
    if (uid == null) {
      return this.http
        .get(
          this.constant.root_url +
            'api/organisationUnits.json?filter=level:eq:1&paging=false&fields=id,name,level,children[id,name,level]'
        )
        .pipe(
          map((response: Response) => response.json()),
          catchError(this.handleError)
