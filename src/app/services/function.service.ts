  ...
  save(sFunction: FunctionObject) {
    return new Observable(observable => {
      if (sFunction.id) {
        ...
        this.http
          .put('dataStore/functions/' + sFunction.id, sFunction)
          .subscribe(
            results => {
            ...
            this.http.get('system/info').subscribe(
              (results: any) => {
                sFunction.href =
                  ...
                  sFunction.id;
                this.http
                  .post('dataStore/functions/' + sFunction.id, sFunction)
                  .subscribe(
                    results => {
                      observable.next(sFunction);
                      observable.complete();
              ...
              forkJoin(this.creationOfNewFunctionObseravble()).subscribe(
                (responses: any) => {
                  this.getAll().subscribe(
                    ...
                    getAllError => {
                      observ.error(getAllError);
                      observ.complete();
                    }
                  );
                },
                createError => {
                  observ.error(createError.json());
                  observ.complete();
                }
              );
            });
