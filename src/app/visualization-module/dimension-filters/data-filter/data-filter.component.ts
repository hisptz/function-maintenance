...
export class DataFilterComponent implements OnInit, OnDestroy {
  ...
  @Output() onDataFilterClose: EventEmitter<boolean> = new EventEmitter<
    boolean
  >();
  ...
  getDataItems() {
    ...
    return {
      ...
      rl: _.flatten(
        _.map(this.dataItems.functions, (functionObject: any) =>
          _.map(functionObject.rules || [], (rule: any) => {
            return {
              id: functionObject.id + '.' + rule.id,
              name: rule.name,
              functionObject: {
    ...
    if (
      ...
      if (group.id === 'ALL') {
        ...
        if (group.hasOwnProperty('dataElements')) {
          const newArray = _.filter(data.de, (dataElement: any) => {
            return _.includes(
