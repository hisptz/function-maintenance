...
export class VisualizationFilterSectionComponent implements OnInit {
  ...
  runOptions = {
    ...
    ManyMany: {
      name: 'Multiple Org Unit & Multiple Period',
      orgUnits: 2,
      periods: 2
    },
    ZeroOne: {
      name: '0 Org Unit & 1 Period',
      orgUnits: 0,
      periods: 1,
      exception: true
    },
    OneZero: {
      name: '1 Org Unit & 0 Period',
      orgUnits: 1,
      periods: 0,
      exception: true
    }
  ...
  updateSelection() {
    ...
    this.currentRunOption.periods =
      this.selectedOu.value.split(';').length > 1
        ? 2
        : this.selectedOu.value.split(';').length;
  ...
  onFilterUpdateAction(filterValue: any, filterType: string) {
    ...
    if (filterType === 'LAYOUT') {
      ...
      const selectionArray = _.filter(
        [this.selectedOu, this.selectedPe, this.selectedDx, this.selectedFn],
        selection => selection
      );
      if (filterType === 'PERIOD') {
        ...
        const dxValues = _.map(
          _.filter(
            filterValue.itemList,
            (item: any) => item.type !== 'FUNCTION_RULE'
          ),
          (item: any) => item.id
        ).join(';');
        this.selectedDx = {
