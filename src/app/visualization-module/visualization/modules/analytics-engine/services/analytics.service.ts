...
export class AnalyticsService {
  constructor(
    private httpClient: AnalyticsHttpClientService,
                  ...
                  this._getAnalyticsObjectFromServer(
                    ...
                    (analyticsResponse: any) => {
                      if (analyticsResponse && analyticsResponse.rows) {
                        observer.next(
                          analyticsHelpers.getMergedAnalytics(
                            _.filter(
  ...
  private _getAnalyticsObjectFromServer(dataDimension: DataDimension) {
    return new Observable(observer => {
      ...
      from(dataDimensionArray)
        .pipe(
          mergeMap((dataDimensionObject: any) =>
  ...
  private _getAnalyticsForNormalDimensions(
    dataDimension: DataDimension
  ): Observable<any> {
  ...
  private _getAnalyticsForFunctionDimensions(
    dataDimension: DataDimension
  ): Observable<any> {
    // get functions id from data
    console.log('Dimension:', dataDimension);
    const functionDimensionValues: any = _.filter(
      _.map(
        _.map(
          _.filter(
            dataDimension.dimensions,
            (dimensionObject: any) => dimensionObject.dimension === 'fn'
          ),
          (functionDimension: any) => functionDimension.value.split(';')
        ),
    ...
    return new Observable(observer => {
      this.functionService
        .get(functionDimensionValues[0].functionId)
        .subscribe((func: any) => {
          let selectedRule;
          ...
          this.functionService.run(parameters, func).subscribe(
            (results: any) => {
